...
```txt
stockfy
├───config                                    # Global configurations (no business logic)
...
└───moduleExample
    ├───application
    │   ├───dto
    │   ├───exception                         # Module-specific application exceptions
    │   ├───port
    │   └───usecase
    ├───domain
    │   ├───event
    ...
    └───presentation
        ├───controller
        └───mapper
        └───ModuleExapleExceptionHandler.java # Module-specific exception handler (when necessary)
```
...
- Cross-cutting concerns must be placed in `shared`.
