...
public class FindAllUsersUseCase {
  ...
  public FindAllUsersResponse<UserListItemResponse> execute(
      String name, UserListType type, Pageable pageable) {
    Page<User> users = userRepository.findAll(normalizeName(name), pageable);
    List<UserListItemResponse> content =
        users.getContent().stream().map(user -> UserListItemResponse.from(user, type)).toList();
