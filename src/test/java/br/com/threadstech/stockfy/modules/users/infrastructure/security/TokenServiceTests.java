...
class TokenServiceTests {
	...
	void consumeRefreshToken_whenTokenDoesNotExist_thenReturnsEmpty() {
		...
		TokenService tokenService = new TokenService(this.redisTemplate, properties, this.hmacSha256Hasher);
		given(this.redisTemplate.opsForValue()).willReturn(this.valueOperations);
		given(this.hmacSha256Hasher.hash("missing-token")).willReturn("hashed-missing-token");
		given(this.valueOperations.getAndDelete("refresh_token:hashed-missing-token")).willReturn(null);
	...
	void consumeRefreshToken_whenRedisValueIsMalformed_thenReturnsEmpty() {
		...
		TokenService tokenService = new TokenService(this.redisTemplate, properties, this.hmacSha256Hasher);
		given(this.redisTemplate.opsForValue()).willReturn(this.valueOperations);
		given(this.hmacSha256Hasher.hash("malformed-token")).willReturn("hashed-malformed-token");
		given(this.valueOperations.getAndDelete("refresh_token:hashed-malformed-token")).willReturn("not-a-uuid");
