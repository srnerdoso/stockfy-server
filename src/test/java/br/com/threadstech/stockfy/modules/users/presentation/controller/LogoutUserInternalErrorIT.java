...
class LogoutUserInternalErrorIT {
  ...
  void logoutUser_whenUnexpectedErrorOccurs_thenReturns500WithoutStacktrace() {
    ...
    doThrow(new RuntimeException("logout database stacktrace detail"))
        .when(logoutUseCase)
    ...
    ResponseEntity<String> response =
        restTemplate.exchange(
            ...
            HttpMethod.DELETE,
            new HttpEntity<>(authenticatedHeaders(refreshToken)),
            String.class);
  ...
  private HttpHeaders authenticatedHeaders(String refreshToken) {
    ...
    headers.add(
        HttpHeaders.COOKIE,
        "access_token=" + jwtService.generateToken(USER_ID, Set.of(UserRole.USER)));
