...
class UnlockUserInternalErrorIT {
	...
	void unlockUser_whenUnexpectedErrorOccurs_thenReturns500WithoutStacktrace() {
		...
		willThrow(new RuntimeException("unlock database stacktrace detail")).given(this.unlockUserUseCase)
			.execute(LOCKED_ID);

		ResponseEntity<String> response = this.restTemplate.exchange("/api/v1/users/{id}/unlock", HttpMethod.PATCH,
				new HttpEntity<>(authenticatedHeaders()), String.class, LOCKED_ID);

		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR);
		MatcherAssert.assertThat(response.getBody(), not(containsString("unlock database stacktrace detail")));
	...
	private String userStatus() {
		return this.jdbcTemplate.queryForObject("SELECT status FROM users WHERE id = ?::uuid", String.class, LOCKED_ID);
	}
